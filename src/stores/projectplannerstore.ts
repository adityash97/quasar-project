...
export default  defineStore('projectplanner', () => {
  ...
  const getCompletedProject = () =>{
    return projectData.value.filter((project : object)=> project.completed == true)
  }


  const getOnGoingProject = () =>{
    return projectData.value.filter((project:object)=> project.completed == false)
  }


  const updateProject =  (projectObject : object,edit:boolean) =>{  // Implement Interface
    if(edit){
  ...
  const deleteProject = (projectObject : object) =>{
    projectData.value = projectData.value.filter((project:object)=> project.id != projectObject.id)
